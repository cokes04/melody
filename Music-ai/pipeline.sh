...
export PREFIX_LAMBDA="sam_templates/generate-music-lambda"
export TEMPLATE_LAMBDA="generate-music-lambda"
export TEMPLATE_REGISTRY="generate-music-registry"
...
do
	case $OPTION in
		...
		r)
            ...
            sam.cmd deploy -t registry.yaml --stack-name ${TEMPLATE_REGISTRY} --capabilities ${CAPABILITIES} --parameter-overrides Project=${TEMPLATE_REGISTRY} Stage=${stage} User=${user} --force-upload --no-confirm-changeset
			;;
        d)
            path=$OPTARG
            path=${path:-.}
            ...
            sam.cmd build -t template.yaml --parameter-overrides DockerPath="${path}"
            echo "Deploying template"
            sam.cmd deploy --stack-name ${TEMPLATE_LAMBDA} --capabilities "CAPABILITY_NAMED_IAM" --parameter-overrides Project="${TEMPLATE_LAMBDA}" Stage=${stage} --s3-bucket ${BUCKET//\"} --s3-prefix ${PREFIX_LAMBDA} --region "${region}" --image-repository "${DOCKER_REGISTRY}/${APP_NAME//\"}" --force-upload --no-confirm-changeset
            ;;
        \?)
